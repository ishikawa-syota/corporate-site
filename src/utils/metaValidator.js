			...
			try {
				...
				if (errors.length > 0) {
					...
					errors.forEach((error) => {
						console.error(
							chalk.yellow(`- ${error.file}: ${error.message}`)
						);
					});
					throw new Error(
...
function validateHtmlContent(html, filePath, errors) {
	try {
		...
		if (!title || !title.text || title.text.trim() === '') {
			...
			title.text.trim() === 'Astro' ||
			title.text.trim() === 'サイト名'
		) {
			errors.push({
				file: filePath,
		...
		// ファビコンのチェック
		const favicon = root.querySelector(
		...
		if (
			...
			!isAbsoluteUrl(ogImage.getAttribute('content'))
		) {
			errors.push({
				file: filePath,
		...
		if (
			...
			errors.push({
				file: filePath,
				message:
					'Twitter Card: <meta name="twitter:card" content="summary_large_image">が設定されていません',
			});
		}
	} catch (error) {
		errors.push({
