...
/news/category/{slug}/…（未実装・予定。カテゴリはslugフィールド運用）
...
- ページサイズは `src/data/config.ts` の `NEWS_PAGE_SIZE`(8) / `VOICES_PAGE_SIZE`(9)。
- 詳細URLにカテゴリを含めない（再分類してもURLが変わらないように）。
- 一覧の描画は `src/section/news/NewsList.astro` / `src/section/voices/VoiceList.astro` に共通化（index と page/[page] の両方から使う）。ページャは `src/components/common/Pager.astro`（**デザインは仮。Figma照合未実施**）。
...
- `/news/category/{slug}/` + ページネーションの実装
- Pagerコンポーネントのデザイン（Figma確認）
