...
async function processHtmlFiles(dir, relativePath = '') {
	...
	for (const file of files) {
		...
		if (stats.isDirectory()) {
			// ディレクトリの場合は再帰的に検索
			await processHtmlFiles(filePath, path.join(relativePath, file));
			...
			try {
				...
				if (updatedContent !== fileContent) {
					...
					// 変更されたクラスを表示（デバッグ用）
					const modifiedClasses = getModifiedClasses(
						fileContent,
						updatedContent
					);
					console.log(
						chalk.yellow(
							`    追加されたクラス: ${modifiedClasses.join(', ')}`
						)
					);
				} else {
					console.log(
						chalk.yellow(`  ⚠ 変更なし: ${relativeFilePath}`)
					);
				}
			} catch (error) {
				console.error(
					chalk.red(
						`  ⚠ 処理エラー: ${relativeFilePath} - ${error.message}`
					)
				);
			}
		}
	...
	try {
		...
		oldElements.forEach((element) => {
			const classes = element.classNames
				.split(' ')
				.filter((cls) => cls.startsWith('analy-'));
		...
		console.error(
			chalk.yellow(`  ⚠ クラス変更検出エラー: ${error.message}`)
		);
		return [];
	}
}
	...
	try {
		...
		ctaButtons.forEach((button, index) => {
			if (!button.id) {
				...
				changes.push(
					`ID: analy-cta-btn${(index + 1).toString().padStart(2, '0')} を .c-btn-cta に追加`
				);
				modified = true;
			}
		});
		...
		submitButtons.forEach((button, index) => {
			if (!button.id) {
				...
				changes.push(
					`ID: analy-form-btn${(index + 1).toString().padStart(2, '0')} をsubmitボタンに追加`
				);
				modified = true;
			}
		});
		...
		textLinks.forEach((link, index) => {
			if (
				!link.id &&
				!link.closest('.l-header') &&
				!link.closest('.l-footer')
			) {
				link.id = `analy-link${(index + 1).toString().padStart(2, '0')}`;
				changes.push(
					`ID: analy-link${(index + 1).toString().padStart(2, '0')} をリンクに追加`
				);
				modified = true;
			}
		});
		...
		if (modified && !root.toString().includes('analy-')) {
			console.log(
				chalk.red(
					'  ⚠ node-html-parserの出力にanaly-クラスが含まれていません。直接HTMLを操作します。'
				)
			);
...
function hasAnalyClass(element, prefix) {
	return Array.from(element.classNames.split(' ')).some((className) =>
